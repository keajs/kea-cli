...
program
  ...
  .usage('<project_name>')
  .option('-t, --template <name>', 'A template to use when creating the project')
  .parse(process.argv)
...
let repo = template.indexOf('/') > 0 ? program.template : templates[template]
...
let fullRepo = `https://github.com/${repo}`
